...
public class sms {
	...
	public static boolean sendVerifyMessage(String mobile, int code) {
		//设置超时时间-可自行调整，单位毫秒
	...
	public static boolean sendApplicationMessage(String mobile, String name, Date start, Date end, String roomname, String password) {
		//设置超时时间-可自行调整
		System.setProperty("sun.net.client.defaultConnectTimeout", "10000");
		System.setProperty("sun.net.client.defaultReadTimeout", "10000");
		//初始化ascClient需要的几个参数
		...
		IAcsClient acsClient = new DefaultAcsClient(profile);
		 ...
		 //可选:模板中的变量替换JSON串,如模板内容为"亲爱的${name},您的验证码为${code}"时,此处的值为
		 //友情提示:如果JSON中需要带换行符,请参照标准的JSON协议对换行符的要求,比如短信内容中包含\r\n的情况在JSON中需要表示成\\r\\n,否则会导致JSON在服务端解析失败
		 String json = "{\"name\":\"" + name + "\",\"start\":\"" + dateformat.timeToString(start) + "\",\"end\":\"" 
				 + dateformat.timeToString(end) + "\",\"room\":\"" + roomname + "\",\"p\":\"" + password + "\"}";
